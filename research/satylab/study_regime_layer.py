...
def main() -> None:
    ...
    A("")
    A(f"**必须马上说清楚的一件事**：全样本本来就是 z_geom = {d_all['z']:+.2f}"
      f"（这是 `V15_ENTRY_LOCATION.md` 已经报过的数）。"
      f"区间态的 {zr:+.2f} 与它几乎没有区别，而区间 vs 趋势的**状态对照**只有 "
      f"{dz:+.2f}。**「区间态显著劣于几何零假设」在算术上是真的，"
      f"但它不是状态的属性——整套系统在哪儿都劣于几何零假设。**"
      f"要证明状态层，需要的是状态对照那一行显著，不是核心格子本身显著。")
    A("")
    A("另外，四个格子里最差的不是「区间」而是 **区间 × Recovery**"
      f"（超额 −9.0 pp，z={grid[(RANGE,'Recovery')]['z']:+.2f}，"
      f"净均R {grid[(RANGE,'Recovery')]['bnet']:+.3f}）；"
      f"区间 × Vomy（n=184）与趋势态没有可分辨的差别。"
      "**「区间」这个标签在这里是两台机器的和，不是一个状态。** 下一节拆开它。")
    A("")
    ...
    A("`区间 = 盒内 OR 未排列` 是一个或，两条腿完全可能一条有效一条无效。"
      "而且 **Vomy 按构造就是「排列破掉之后」才发火**，它落在「未排列」这条腿上的"
      "概率天然远高于 Recovery（实测 39% vs 8%）——"
      "不拆开看，整张表会把 **setup 差异**误读成**状态差异**。"
      "同一个理由也适用于止损：Vomy 的保护位是 10 根极值，Recovery 的是回踩低点，"
      "**两者的 D4 天生差一倍以上（0.19 vs 0.09 ATR）**，"
      "所以「状态」「setup」「止损距离」这三件事在 v14 的数据里本来就是缠在一起的。")
    A("")
    A(HDR)
    A(SEP)
    legs = {}
    ...
    A("### 4.1 这一节的结果与假设**只对了一半，另一半正好相反**")
    A("")
    A("假设描述的几何——「跌到区间底才叉得下来」——**对延续型交叉（Recovery）"
      "是精确成立的，而且强度惊人**：盒内 Recovery 的空头信号有 **91%** 落在盒的下半"
      "（u 均值 −0.377），多头信号有 **82%** 落在上半（u 均值 +0.358），"
      "有向位置 v 的 t = **+8.42**、Wilcoxon z = **+6.43**。"
      "这是本报告里唯一越过 Bonferroni 门槛的量。")
    A("")
    A("**但对翻转型交叉（Vomy）方向完全相反**：盒内 Vomy 的空头信号 78% 落在盒的"
      "**上**半（u 均值 +0.296），多头 58% 落在**下**半（u 均值 −0.154），"
      "v 的 t = **−6.00**。Vomy 按构造就是「排列破掉、回抽 13 再打回去」，"
      "它天然是在**冲高之后**做空、在**杀跌之后**做多——位置反而是好的。")
    A("")
    A("两台机器一正一反，合起来 v 均值 +0.022（t=+0.64）。"
      "**「区间态里交叉类必然在最差位置发火」这句话，把两台性质相反的机器"
      "当成了一台。** 它对 v14 一半的信号成立，对另一半是反的。")
    A("")
    A("**更关键的是：位置成立，收益不跟着走。** 在盒内 Recovery 组内部，"
      "v 越大（位置越差）**并不**对应更差的结果——超额的趋势检验 z = −0.03，"
      "净R 秩相关 ρ = +0.052。也就是说，即使几何上信号确实钉死在盒的坏那一半，"
      "**这个「坏位置」本身并没有在数据里换算成更坏的结果**。"
      "「符号论」需要的正是这一步，而这一步没有出现。")
    A("")
    ...
    A(f"- ⚠ **但边缘拒绝并不赚钱**：括号净均R {st.mean([bnet(s) for s in ge]):+.3f}，"
      ...
      f"换规则换来的是少亏，不是转正。"
      f"毛括号均R {st.mean([br(s) for s in ge]):+.3f} 已经接近打平，"
      f"净均R 变成 {st.mean([bnet(s) for s in ge]):+.3f}——"
      f"**差额全部是 0.6 点点差**，因为它的止损只有 0.093 ATR ≈ 9 点。")
    ndays = len(set(b.day for b in bars))
    A(f"- **频率警告**：边缘拒绝在 {ndays} 个交易日里出了 {len(edges)} 个信号"
      f"（**{len(edges)/ndays:.1f} 个/日**），比 v14 的交叉类"
      f"（{len(sigs)/ndays:.1f} 个/日）少一半，但**仍然是 churn 量级**。"
      f"这个方案的出处（Saty 07-28）当天**只做了两次**，而且是在他先"
      f"「sit on hands / be more picky」定性之后。"
      f"所以「区间态改做边缘拒绝」在语料里对应的是"
      f"**每天 1–2 次**，不是每天 4.7 次——"
      f"要把它变成他那个东西，还缺「今天只看这 2–3 条位」与"
      f"「同一位第 N 次触及」这两层筛选，二者本报告都没有测。")
    if len(cedges) >= 5:
    ...
    A("")
    A("**这张表先把那个「朴素解释」在状态层面直接否掉了**：")
    A("")
    A("- 区间态的止损**不是更小，是更大**（0.151 vs 0.137 ATR），日 ATR 也几乎一样"
      "（94.9 vs 97.3 点）。所以「区间态波动小、止损小」这个说法"
      "**在 v15.1 的状态定义下是错的**——因为「未排列」这条腿带来的是 Vomy，"
      "而 Vomy 的止损最大。")
    A("- 但**在盒内/盒外这条腿上，朴素解释是对的**：盒内 0.137 vs 盒外 0.153 ATR，"
      "而且盒内 × Recovery 的 0.081 ATR 是全样本最小的一格。")
    A("- **还有一个必须点名的机制**：区间态的 |T1−entry| 是 0.115 ATR，趋势态只有 "
      "0.073 ATR——盒内价格离下一条具名位天然更远，所以区间态的几何零假设"
      "（0.543）本来就比趋势态（0.624）低得多。"
      "**这正是为什么原始命中率 49.3% vs 60.6% 那个 11 个点的差距几乎全是假的**，"
      "扣掉几何零假设之后只剩 −5.1 pp vs −1.7 pp。"
      "任何不用几何零假设看这张表的人都会得出「区间态惨不忍睹」的错误结论。")
    A("")
    ...
    A("")
    A("**这张表怎么读——这是本报告最需要说清楚的一段：**")
    A("")
    A("- 「盒内×Recovery **vs 其余全部**」：控制 D4 之后，超额得分 z 从 −1.99 掉到 −1.22，"
      "**净钱差从 −19.5 掉到 −5.5（z 从 −1.79 掉到 −0.74）**。"
      "也就是说，与「其余全部」的差距里，**钱的那一部分大半是止损距离解释掉的**"
      "——「其余全部」里塞满了 Vomy，而 Vomy 的止损天生是 Recovery 的两倍多"
      "（0.19 vs 0.09 ATR）。这个对比本身就是被 D4 污染的，不该当作状态层的证据。")
    A("- 「**Recovery 内部**：盒内 vs 盒外」才是干净的对比（同一台机器，D4 只差 "
      "0.081 vs 0.094）。控制 D4 之后超额 z 从 −2.16 只降到 −1.84，"
      "净均R 差从 −0.316 降到 −0.266（z −2.29 → −2.01），"
      "**但净钱差从 −23.3 掉到 −10.2（z −2.06 → −1.37）**。"
      "5 个 D4 层里有 4 层盒内更差。")
    A("- 「Vomy 内部：盒内 vs 盒外」**完全没有效应**（z=+0.27，控制后 +0.35）。")
    A("")
    A("**所以对「是不是伪发现」这个问题，诚实的回答是「一半一半」**：")
    A("")
    A("1. 「**区间**态整体更差」这个说法，控制与否都不显著（z −0.79 / −1.00），"
      "而且它的 Vomy 那一半根本没有效应。**作为状态层的证据，它是伪的。**")
    A("2. 「**盒内的延续型交叉**更差」在控制 D4 之后**没有消失**"
      "（超额 z −2.16 → −1.84），但**它的「钱」那一半消失了一大半**"
      "（净钱 z −2.06 → −1.37）。命中率上还剩一点东西，钱上剩得不多。")
    A("3. 两者都远不到 Bonferroni 门槛。**本样本分不开「位置」与「刻度」。**")
    A("")
    ...
    A("")
    A(f"**常规 |z|>1.96 在这个 family size 下没有意义。** 本报告里"
      f"**没有任何一个关于收益的 z 越过 {thr:.2f}**"
      f"（最大的两个是 §6.3 的净均R 差 −2.92 与 §2.2 的"
      f"「盒内×Recovery」z_geom = −2.63），所以关于收益的判决只能是"
      f"「**没找到证据**」或「**方向与假设相反**」，不能是「证明了反面」。")
    A("")
    A("唯一越过门槛的是 §4 的**位置分布**（Recovery t=+8.42 / Vomy t=−6.00）。"
      "但那是**信号几何的恒等式**——"
      "「收盘刚叉回 13、而 13 在盒中偏某一侧」当然会把信号钉在盒的某一半——"
      "**它不是一个关于收益的发现**，不能拿它当状态层成立的证据。"
      "恰恰相反：几何成立、收益不跟着走，正是「符号论」最难解释的地方。")
    A("")
    ...
    A("## 8 · 与假设相反的格子（如实单列）")
    A("")
    A("纪律 7。下面每一条的方向都**与状态层假设相反**，一个不删：")
    A("")
    gsp_r = [s for s in csigs if s.state == RANGE]
    gsp_t = [s for s in csigs if s.state == TREND]
    zgr = z_geom([s.hit for s in gsp_r if s.hit is not None],
                 [s.pnull for s in gsp_r if s.hit is not None])[0]
    zgt = z_geom([s.hit for s in gsp_t if s.hit is not None],
                 [s.pnull for s in gsp_t if s.hit is not None])[0]
    A(f"1. **^GSPC RTH 对照里，区间态比趋势态好**：区间 z_geom = {zgr:+.2f}"
      f"（净均R +0.071，唯一为正的那一格），趋势 z_geom = {zgt:+.2f}"
      f"（净均R −0.127）。虽然只是方向性对照（n=64/87，且位价不可代理），"
      f"但它的方向是**完全相反**的。")
    A("2. **趋势态的 Vomy 比区间态的 Vomy 更差**（超额 −3.8 vs −2.6 pp）。"
      "假设预测趋势态应当全开、区间态应当禁用；数据里 Vomy 在哪个状态都一样，"
      "而且趋势那一格还稍差。")
    A("3. **「盒外 · 未排列」是全表最好的一格**（超额 +0.7 pp，z=+0.11，"
      "净钱 +1.2，是唯一净钱为正的格子）——而按 v15.1 的定义它属于**区间态**，"
      "是要被禁用的。")
    A("4. **Vomy 的位置分布与假设完全相反**（§4）：v 均值 −0.237，t=−6.00。"
      "翻转类交叉是在盒的**对侧**发火的，位置反而是好的。")
    A("5. **在盒内 Recovery 组内部，位置越差并不对应结果越差**"
      "（超额趋势检验 z=−0.03，净R 秩相关 ρ=+0.052）。"
      "假设的因果链「坏位置 ⇒ 坏结果」，在最支持它的那一格里也断了。")
    A("6. **反做的好处在 D4 中高档消失甚至转负**（§6.2 末表：Q3 −0.072、Q4 −0.139）。"
      "如果符号真的是反的，止损大小不该改变符号。")
    A("")
    A("四个格子 + 两个合计，按超额从好到差排序：")
    A("")
    ...
    rec_box = [s for s in sigs if s.in_box and s.setup == "Recovery"]
    vom_box = [s for s in sigs if s.in_box and s.setup == "Vomy"]
    v_rec = tstat([s.v for s in rec_box])
    v_vom = tstat([s.v for s in vom_box])
    dn_int, zn_int = mean_diff_z(pd_net(rng), pd_net(trd))
    zedge = z_geom([s.hit for s in ge], [s.pnull for s in ge])[0]
    zgc = z_geom([s.hit for s in gc], [s.pnull for s in gc])[0]
    A(f"| 推论 | 数字 | 判定 |")
    A("|---|---|---|")
    A(f"| P1 区间×交叉类劣于几何零假设 | z_geom = {zr:+.2f}"
      f"（n={grid[(RANGE,'ALL')]['res']}）；但**全样本本来就是 "
      f"{d_all['z']:+.2f}**，区间 vs 趋势的状态对照 z = {dz:+.2f} | "
      f"**不成立**：这不是状态效应，是整套系统的基线负值 |")
    A(f"| P2 反着做更好（**最硬的一条**） | 区间配对超额差 t = {t_dex:+.2f}；"
      f"区间与趋势的**交互** z = {zn_int:+.2f}（净R）| "
      f"**不成立**：反做在哪儿都好一点点，区间并不特殊 |")
    A(f"| P3 空在盒下半 / 多在盒上半 | 合并 v 均值 "
      f"{st.mean([s.v for s in inbox]):+.3f}，t = {v_t:+.2f}；"
      f"**Recovery t = {v_rec:+.2f}，Vomy t = {v_vom:+.2f}** | "
      f"**一半成立、一半完全相反**：几何直觉只对延续类成立，"
      f"翻转类的位置正好是反的 |")
    A(f"| P4 边缘拒绝优于同期交叉类 | 边缘拒绝 z_geom = {zedge:+.2f}，"
      f"盒内交叉类 {zgc:+.2f}；"
      f"两者差 z = {zxx:+.2f}（超额）/ {zde:+.2f}（净R）| "
      f"**方向对，强度不够**；且边缘拒绝自己也不赚钱（总净R "
      f"{sum(bnet(s) for s in ge):+.1f}） |")
    A("")
    A("### 9.1 一句话")
    A("")
    A("**「区间态里交叉类的符号是反的」这个断言，没有通过检验。**"
      "支撑它的最硬证据（反过来做会更好）在区间态里 t=+0.97，"
      "而且在趋势态里也差不多大——**反做的好处不是区间态特有的，"
      "它只是「一个负期望系统反过来做会少亏」的普遍现象**。")
    A("")
    A("### 9.2 但检验挖出了一个更具体、也更可能是真的东西")
    A("")
    A("把「区间」这个标签拆开之后，负担几乎全部集中在**一格**：")
    A("")
    A("| | n | 超额pp | z_geom | 括号净均R | 净钱(ATR)×1000 | 均D4 | 有向位置 v |")
    A("|---|---|---|---|---|---|---|---|")
    for lbl, g in (("**盒内 × Recovery（交叉·延续）**", rec_box),
                   ("盒内 × Vomy（交叉·翻转）", vom_box),
                   ("盒外 × Recovery", [s for s in sigs
                                        if not s.in_box and s.setup == "Recovery"]),
                   ("盒外 × Vomy", [s for s in sigs
                                    if not s.in_box and s.setup == "Vomy"])):
        d = [s for s in g if s.hit is not None]
        if len(d) < 5:
            continue
        _, _, ob, nb2 = z_geom([s.hit for s in d], [s.pnull for s in d])
        zz = z_geom([s.hit for s in d], [s.pnull for s in d])[0]
        A(f"| {lbl} | {len(g)} | {100*(ob-nb2):+.1f} | {zz:+.2f} | "
          f"{st.mean([bnet(s) for s in d]):+.3f} | "
          f"{1000*st.mean([bmoney(s) for s in d]):+.1f} | "
          f"{st.mean([s.d4 for s in g]):.3f} | {st.mean([s.v for s in g]):+.3f} |")
    A("")
    A("这一格同时满足三件事：**位置最差（v=+0.37，t=+8.4，"
      "空信号 91% 落在盒下半、多信号 82% 落在盒上半）、止损最小（0.081 ATR，"
      "正好是 `V15_ENTRY_LOCATION.md` 里唯一越过 Bonferroni 的杀手区间）、"
      "结果最差**。三件事同时出现，但本报告的样本量**分不开**它们"
      "（控制 D4 后的对照见 §6.3）。")
    A("")
    A("### 9.3 落到规则上应该怎么办")
    A("")
    A("1. **不要按「区间/趋势」整体开关交叉类。** 状态对照 z=−0.79，"
      "区间态里 Vomy 那一半（n=184，净均R −0.118）与趋势态没有可分辨的差别，"
      "关掉它是纯粹的伤害。这与 `REGIME_MONTHLY_BOX_2026-07-25.md` 的教训是同一条。")
    A("2. **不要实现「区间态反着做」。** P2 不成立，而且反做后净均R 仍是负的"
      "（区间 −0.094，全样本 −0.076）——反做只是把大亏变小亏，不是转正。")
    A("3. **v15.1 第 0 层写的「区间态禁用交叉类」应当降级**："
      "如果一定要留一条基于本报告的规则，它的正确形状是"
      "**「盒内 + 延续型交叉 + 风险距离 < 0.09 ATR」三条同时成立时不下注**——"
      "而这三条里已经被独立验证过的只有第三条（最小风险距离 0.12 ATR，"
      "本来就在 v15 里开着）。**前两条在本样本里加不出增量证据。**")
    A("4. **边缘拒绝值得继续查，但不是因为它赢了**，"
      "而是因为它在止损更小的情况下没有更差（z_geom "
      f"{zedge:+.2f} vs 盒内交叉类 −2.21），且它的失败模式与交叉类不相关。"
      "要判决它需要真实 CFD 历史和更长样本。")
    A("")
    A("### 9.4 这份报告没能回答的")
    A("")
    A(f"- 59 个交易日、517 个信号。盒内×Recovery 那一格只有 105 笔。"
      f"**Bonferroni 门槛 {thr:.2f}；所有关于收益的 z 都在 ±3.0 以内，全部不显著。**"
      f"唯一越过门槛的是位置分布（§4），而那是几何恒等式不是收益发现。")
    A("- 盒内/盒外的划分依赖位价，而我们手上没有 CAPITALCOM:SPX500 的真实历史"
      "（纪律 5）。ES=F 只是作息一致。**一部分「盒内」信号在真实图表上并不在盒内。**")
    A("- 状态层用的是 v14 的信号集。v15 的 S2（位拒绝）/ S3（背离）"
      "在这个样本里根本不存在，所以「区间态改用边缘拒绝与背离」这个方案"
      "**只有一半被测到**（§5 的边缘拒绝），背离那一半没测。")
    A("")
