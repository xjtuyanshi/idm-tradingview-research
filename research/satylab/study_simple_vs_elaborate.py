...
def main() -> None:
    ...
    def boot_band(n_target: int, reps: int = BOOT):
        ...
        return {"tot_lo": q(tots, 0.05), "tot_hi": q(tots, 0.95),
    ...
    for d in (dA, dB, dC, dD, dDl, dE):
        ...
        A(f"| {d['label']} | {ns} | {1000*ns/nb:.1f} | 每 {nb/ns:.1f} 根一笔 | "
          f"{ns/days_span:.1f} |")
    A("")
    A(f"**A 比 v14 更churn。** A 每 {nb/dA['n_sig']:.1f} 根 K 就要下一次单，v14 是每 "
      f"{nb/dD['n_sig']:.1f} 根。「规则少」和「动手少」是两件事，把 v14 的闸门全部拆掉之后，"
      f"剩下的极简规则触发得更频繁——这正是 v14 当初长出那些闸门的动机之一。")
    A("")
    ...
    A("")
    A("| 选手 | n | 均毛R | 均净R | 点差成本(R) | 点差成本占|均毛R| |")
    A("|---|---|---|---|---|---|")
    for d in (dA, dB, dC, dD, dE):
        cost = d["avg_r"] - d["avg_net"]
        pct = 100 * cost / abs(d["avg_r"]) if d["avg_r"] else float("nan")
    ...
    A("")
    A("| 选手 | 每笔平均亏 (ATR) | 每笔平均亏 (点，按 88 点 ATR) | 全样本 60 天累计 (ATR) |")
    A("|---|---|---|---|")
    for d in rank:
        A(f"| {d['label']} | {d['avg_money']:+.4f} | "
          f"{88*d['avg_money']:+.2f} | {d['tot_money']:+.2f} |")
    A("")
    A(f"（这一列假设每笔的名义敞口相同。如果按固定 R 风险下注，那就看均净R 那一列——"
      f"但那样 B 的「每笔只冒 {dB['risk_atr']:.3f} ATR 风险」会被放大成和 D 的 "
    ...
    A("")
    A("**你是对的。那些额外的东西没有买到任何东西。**")
    A("")
    A("我把你那句「符合直觉」的版本真的写出来了——四条规则，一句话能说完：看价格在不在"
      "盒子里、只在位附近动手、区间朝里趋势顺势、目标和止损都用位。然后拿它和现在这套"
      "二十多个参数的 v14 放在同一份数据上跑同样的判定。结果是：")
    A("")
    A(f"1. **两个都在亏，而且亏得差不多。** 四条规则的版本每笔亏 "
      f"{88*dA['avg_money']:.2f} 点，v14 全套每笔亏 {88*dD['avg_money']:.2f} 点。"
      f"多出来的十几个参数没有把亏损变小。")
    A(f"2. **更难受的是：随机开仓亏得比两个都少。** 在同样的 K 上扔硬币决定多空、用同样"
      f"的目标和止损，每笔亏 {88*dE['avg_money']:.2f} 点。也就是说，无论四条规则还是"
      f"二十条规则，**都没有比扔硬币提供更多的信息**。这不是我挑话说，这是这份 60 天"
      f"数据上的直接测量。")
    A(f"3. **v14 甚至明显比随机更差。** 同样做 {dD['n']} 笔，随机的总账中位数是 "
      f"{bandD['tot_md']:+.1f}R，90% 的随机结果落在 [{bandD['tot_lo']:+.1f}, "
      f"{bandD['tot_hi']:+.1f}]R 之间；v14 实测 {dD['tot_net']:+.1f}R，掉在这个范围的"
      f"下面。那些闸门和分级不是没用，是**倒扣分**。")
    A(f"4. **「规则少」不等于「动手少」。** 四条规则的版本 {dA['n_sig']} 个信号，"
      f"v14 是 {dD['n_sig']} 个——极简版反而更频繁地叫你下单。你付的点差更多。"
      f"这就是当初那些闸门为什么会长出来：它们是在压交易次数，而不是在提高胜率。")
    A("")
    ...
    A("")
    A("**所以下一步该怎么办。** 不是继续加规则，也不是回去调这四条规则的数字——那两条路"
      "这份报告都试过了，都不动。真正的问题在更上游：**在这个 10 分钟周期、用这一套位"
      "做目标和止损，入场那一刻本身就没有优势**。一件没有优势的事，用四条规则做和用"
      "二十条规则做，结果都是亏点差。要么换周期，要么换赔率结构（比如目标放到两个位以外、"
      "止损收到位的另一侧），要么接受这套东西不成立。继续雕规则是在装修一间没有地基的房子。")
    A("")
    A("**一句话：你的直觉没错，复杂度在这份数据上是纯成本。但把它砍到四条规则也不会"
      "赚钱——因为亏的不是复杂度，是这个入场本身。**")
    A("")
