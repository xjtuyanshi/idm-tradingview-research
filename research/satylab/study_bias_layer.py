...
def interaction(sigs: list[Sig], out: list[str], label: str) -> None:
...
def main() -> None:
    ...
    contrast_table(res1h, sig1h, o)
    res10 = bias_table(sig10, o, "S1 · ES=F 10m 60d（偏向几乎不翻，看看就好）")
    contrast_table(res10, sig10, o)
    ...
    A("## 8 · 多重比较")
    A("")
    A(f"全文共检视 **{CELLS} 个格子**（分组格、对比行、分解行、状态交互格、"
      f"持续性格、日线格、赛跑格）。Bonferroni 门槛 |z| > **{_bonf_z(CELLS):.2f}**"
      f"（α=0.05 双侧）。")
    A("")
    A(f"这意味着：常规 |z| > 1.96 在这个 family size 下**没有意义**。"
      f"本报告里没有任何一个格子越过 {_bonf_z(CELLS):.2f}，"
      f"这一点必须在读任何一张表之前先记住。")
    A("")
    ...
    A("3. **S2 的路径分辨率**：1h 没有更细的数据，同一根 K 内两侧都被触到的交易"
      "一律记未裁决（纪律 3）。未裁决率在表里逐格报出；如果两组未裁决率差很多，"
      "命中率的对比本身就有选择偏差。")
    A("4. **日聚类**：所有关键 z 都按交易日聚类。朴素 z 一律不作数。")
    A("5. **样本量**：S1 约 50 个交易日、S2 约 600 个交易日。"
